...
function getUrgencyColor(urgency: string | null, completed: number) {
...
function buildMarkedDates(events: Event[], selected: string) {
  ...
  events.forEach((evt) => {
    const color = evt.completed ? COMPLETED_COLOR : getUrgencyColor(evt.urgency, 0);
...
export default function CalendarScreen() {
  ...
  const renderEvent = ({ item }: { item: Event }) => {
    ...
    const barColor = getUrgencyColor(item.urgency, item.completed);
    return (
      <View style={[styles.eventCard, isCompleted && styles.eventCardCompleted]}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          {/* En vista "todos" mostramos la fecha sobre el título */}
          {viewMode === 'all' && (
...
const styles = StyleSheet.create({
  ...
  sortLabel: { fontSize: 11, color: Colors.primary, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  listContainer: {
  ...
  dayTitle: { fontSize: 15, fontWeight: '600', color: Colors.text, textTransform: 'capitalize', flex: 1 },
  ...
  eventDate: { fontSize: 11, color: Colors.textSecondary, fontWeight: '500', marginBottom: 2 },
  ...
  completedBadgeText: { fontSize: 11, fontWeight: '600', color: '#4CAF50' },
